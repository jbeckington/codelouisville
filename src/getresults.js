...
function getit(form) {
    ...
    onDemand.setAPIKey('1e062b37c4022b2292f2a806a7d45b8e');
    onDemand.setJsonP(true);
//onDemand.setBaseUrl('http://localhost:8080/ondemand');

    onDemand.getQuote({symbols: form.inputbox.value}, function (err, data) {
        if (err) {
            console.error(err);
        } else {
                ...
                var quotes = data.results;
                    if (quotes == null) {
                        document.getElementById("output").innerHTML = "Please Enter a valid Ticker symbol";
                    } else {
                    }
                    for (x in quotes) {

                        // console.log("getQuote: " + quotes[x].symbol + " [" + quotes[x].name + "] = " + JSON.stringify(quotes[x]));
                        document.getElementById("c.name").innerHTML = "Company Name: "+quotes[x].name
                        document.getElementById("c.price").innerHTML = "Last Price: " + quotes[x].lastPrice;
                        document.getElementById("c.change").innerHTML = "Net Change: " + quotes[x].netChange;
                        document.getElementById("c.percent").innerHTML = "Percent Change: " + quotes[x].percentChange;
